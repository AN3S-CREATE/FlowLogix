...
const UPDATED_AT_TABLES = ['organizations', 'boards', 'lists', 'cards', 'comments'];
...
export class EnableRlsAndTriggers1783940500000 implements MigrationInterface {
    ...
    await queryRunner.query(`
      ...
      BEGIN
        IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = '${APP_ROLE}') THEN
          CREATE ROLE "${APP_ROLE}" WITH LOGIN PASSWORD '${APP_ROLE_PASSWORD}' NOSUPERUSER NOCREATEDB NOCREATEROLE;
        END IF;
      END
      $$;
    `);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DROP POLICY IF EXISTS boards_tenant_isolation ON boards`);
    ...
    for (const table of UPDATED_AT_TABLES) {
      await queryRunner.query(`DROP TRIGGER IF EXISTS set_${table}_updated_at ON "${table}"`);
    }
    await queryRunner.query(`DROP FUNCTION IF EXISTS set_updated_at`);

    await queryRunner.query(`REVOKE ALL PRIVILEGES ON ALL TABLES IN SCHEMA public FROM "${APP_ROLE}"`);
    await queryRunner.query(
