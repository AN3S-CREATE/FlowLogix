...
Phase 2 Core Hardening — **complete** (2026-07-20). SPA JWT + REST hydration, optimistic DnD → API with rollback, `needsResync` → targeted refetch. Estimated readiness **~76–80/100** (from Phase 1 ~68–70). Phase 1 commit `bf50683`; Phase 2 committed separately.
...
- Insight 8: Sync v1 merges content fields only; `position_idx` / offline inserts out of scope (Phase 3).
...
- Phase 0/1 surfaces (auth, health, filters, compose, READMEs)
- `frontend/src/api/*`, `AuthGate`, `useBoardStore`, `persistence`, `Board.tsx`, `useBoardSocket`
- `backend/src/cards/cards.service.ts`, `update-card.dto.ts`, `board-members.service.ts`
- `.index/module-summaries/phase0-readiness.md`, `phase1-quick-wins.md`, `phase2-core-hardening.md`
...
- Q4: When to schedule Atlaskit DnD migration vs sync position_idx work?

## Decisions Made & Rationale
- Decision: Remap FlowLogix Mongo to host port 27018.
  Rationale: Preserve active `chat-mongodb` on 27017.
- Decision: Phase 0 baseline 60/100; Phase 1 = P2–P4 (+ light P6/P9); Phase 2 = P1+P5.
  Rationale: Incremental readiness; SPA rewrite after HTTP hardening.
...
1. Human review of Phase 2 PR/diff on `main`.
2. Phase 3: sync `position_idx` + offline inserts (P7).
3. Separate PR for major dep upgrades (audit critical/high).
4. Optional: Atlaskit pragmatic DnD (P8).
...
- Pattern: Rules/docs converge toward code in Phases 1–2; pragmatic-dnd still pending.
