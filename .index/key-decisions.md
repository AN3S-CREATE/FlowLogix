...
| 2026-08-01 | Keep one workflow file but split responsibilities into separate jobs, with `CI Health` as the branch-protection target | Minimizes workflow sprawl while making required verification explicit and PR-safe |
| 2026-08-01 | Run CI installs with root `npm ci` instead of per-workspace `npm install` | Forces GitHub Actions to validate the committed workspace lockfile and improves reproducibility/caching |
